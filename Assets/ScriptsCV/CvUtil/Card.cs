...
namespace OpenCVForUnitySample
{
    ...
    public class Card
    {
        ...
        public double m_maxSquareLenRatio;
        ...
        public Card(CardType cardType)
        {
           ...
           // m_Knn = new KNearest();
            ...
            if (((int)cardType & (int)CardType.Digit) > 0)
            {
                ...
                // Sample = ReadData("Assets/cards/50/digit_TrainingData.yml", Sample, true);
                // Response = ReadData("Assets/cards/50/digit_LabelData.yml", Response, false);
            }
        ...
        private List<List<Point>> FilterSquares(List<List<Point>> squares)
        {
        ...
        public List<List<Point>> Recgonize(Mat image, out List<int> resultVec, out List<List<Point>> resultSquares, bool one = false)
        {
            one = false;
            ...
            resultSquares = new List<List<Point>>();

            squares = FigProc.FindSquares(image.clone());
            filterSquares = FilterSquares(squares);
            ...
            for (int i = 0; i < filterSquares.Count; i++)
            {
                ...
                if (distance.get(0, 0)[0] < m_distanceThreshold)
                {
                    ...
                    else
                    {
                        ...
                        {
                            resultVec.Add(value);
                            resultSquares.Add(filterSquares[i]);
                        }
                    }
