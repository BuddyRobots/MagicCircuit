...
namespace OpenCVForUnitySample
{
    ...
    public class FigProc
    {
        ...
        public static double FindLen(Point p1, Point p2, bool sqrt_v = true)
        {
            float v = Mathf.Pow((float)(p1.x - p2.x), 2) + Mathf.Pow((float)(p1.y - p2.y), 2);
            return sqrt_v ? Mathf.Sqrt(v) : v;
        }

        public static float Angle(Point pt1, Point pt2, Point pt0)
        {
        ...
        public static List<List<Point>> FindSquares(Mat gray)
        {
            List<List<Point>> squares = new List<List<Point>>();
            List<MatOfPoint> contours = new List<MatOfPoint>();

            Imgproc.findContours(gray, contours, new Mat(), Imgproc.RETR_LIST, Imgproc.CHAIN_APPROX_SIMPLE, new Point(0, 0));
            ...
            for (int i = 0; i < contours.Count; i++)
            {
                Imgproc.approxPolyDP(contours2f[i], approx2f, Imgproc.arcLength(contours2f[i], true) * 0.02, true);
        ...
        public static bool IsSquareClockwise(List<Point> square)
        {
            bool clockwise;
            int direction;
