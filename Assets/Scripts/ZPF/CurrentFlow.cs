...
namespace MagicCircuit
{
    public class CurrentFlow : MonoBehaviour
    {
		...
		private Connectivity[,] reOrderedConn;  // Connectivity for the whole circuit when all switches on after flipComponent and flipLines
        ...
        public bool compute(List<CircuitItem> _circuitItemList, int _level)
        {
			...
			if (removeOpenedCircuit())
				Debug.Log("CurrentFlow_new.cs compute() : have open circuit!");
			...
			{
				Debug.Log("CurrentFlow_new.cs compute() : This circuit can work.");
			}
			else
			{
			...
			for (var i = 0; i < circuitBranch.Count; i++)
			{
				for (var j = 0; j < circuitBranch[i].Count; j++)
					Debug.Log("CurrentFlow.cs compute() : " + circuitBranch[i][j]);
				Debug.Log("CurrentFlow.cs compute() : ------");
			}
			...
			Array.Copy(connectivity, reOrderedConn, connectivity.Length);
        ...
        private void initMembers()
		{
			...
			reOrderedConn = new Connectivity[count, count];
		...
		private void switchOn(int ID)
		{
			for (var i = 0; i < count; i++)
			{
				connectivity[ID, i] = reOrderedConn[ID, i];
				connectivity[i, ID] = reOrderedConn[i, ID];
			}
		}
    }
}